...
local theme
  ...
  # NOTE: Checking for the value of @kanagawa_window_tabs_enabled
  local wt_enabled
  wt_enabled="$(get_tmux_option "@kanagawa_window_tabs_enabled" "off")"
  readonly wt_enabled

  local right_separator
  right_separator="$(get_tmux_option "@kanagawa_right_separator" "")"
  readonly right_separator

  local left_separator
  left_separator="$(get_tmux_option "@kanagawa_left_separator" "")"
  readonly left_separator

  local user
  user="$(get_tmux_option "@kanagawa_user" "off")"
  readonly user

  local host
  host="$(get_tmux_option "@kanagawa_host" "off")"
  readonly host

  local date_time
  date_time="$(get_tmux_option "@kanagawa_date_time" "off")"
  readonly date_time
 ...
 #setw -g window-status-current-format "#[fg=colour232,bg=$thm_orange] #I #[fg=colour255,bg=colour237] #(echo '#{pane_current_path}' | rev | cut -d'/' -f-2 | rev) "
  ...
  # NOTE: With the @kanagawa_window_tabs_enabled set to on, we're going to
  ...
  setw mode-style "fg=${thm_pink} bg=${thm_altblack} bold"
